...
플레이어 전용 보상을 활성화한 경우 전체 선택 순서는 `전용 고정 > 공용 고정 > 전용 범위 > 공용 범위 > 전용 플러스 > 공용 플러스`입니다. 기본적으로 후원 이벤트 하나에는 이 순서로 선택된 보상 하나만 실행됩니다. 단, `streamer-rewards.yml` 전용 보상에 `allowDuplicate: true`를 넣으면 전용 보상 실행 후 매칭되는 공용 보상도 1개 이어서 실행됩니다.
